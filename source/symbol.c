...
#if FUZZY_DEFTEMPLATES
#define AVERAGE_FUZZY_VALUE_SIZE (sizeof(float) * 20)
#endif
...
#if FUZZY_DEFTEMPLATES
    SymbolData(theEnv)->FuzzyValueTable = (CLIPSFuzzyValue **)
                    gm2(theEnv,(int) sizeof (CLIPSFuzzyValue *) * FUZZY_VALUE_HASH_SIZE);
#endif
...
static void DeallocateSymbolData(
  Environment *theEnv)
  {
...
#if FUZZY_DEFTEMPLATES
   CLIPSFuzzyValue *fvhPtr, *nextFVHPtr;
#endif
...
#if FUZZY_DEFTEMPLATES
       (SymbolData(theEnv)->FuzzyValueTable == NULL) ||
#endif
         ...
         if (! fvhPtr->permanent)
           {
            ...
            rtn_struct(theEnv,clipsFuzzyValue,fvhPtr);
           }
         fvhPtr = nextFVHPtr;
        }
     }
...
#if FUZZY_DEFTEMPLATES
   genfree(theEnv,SymbolData(theEnv)->FuzzyValueTable,(int) sizeof (CLIPSFuzzyValue *) * FUZZY_VALUE_HASH_SIZE);
#endif
#endif
   ...
   if (SymbolData(theEnv)->FuzzyValueArray != NULL)
     rm(theEnv,SymbolData(theEnv)->FuzzyValueArray,sizeof(CLIPSFuzzyValue *) * SymbolData(theEnv)->NumberOfFuzzyValues);
#endif
#endif
  }
...
#if FUZZY_DEFTEMPLATES
/*******************************************************************/
/* CreateFuzzyValue:  Searches for the fuzzy value in the hash table  */
/*   If it is already in the hash table, then the address of the   */
/*   fuzzy value is returned.  Otherwise, the it's hashed into the */
/*   table and the address of the fuzzy value is returned.         */
/*******************************************************************/
CLIPSFuzzyValue *CreateFuzzyValue(
   Environment *theEnv,
   struct fuzzy_value *fv)
   {
    ...
    /*=========================================*/
    /* Get the hash value for the fuzzy value. */
    /*=========================================*/
    ...
    /*  for this hash location.  If it is found,        */
    /* then return the address of the fuzzy value.      */
    /*==================================================*/

    while (peek != NULL)
      {
        struct fuzzy_value *peekfv = peek->contents;

        /* Deftemplates associated with the fuzzy value, the
           fuzzy value names, plust the  array sizes and array
           contents must all be the same if they are identical
           fuzzy values;
           When fuzzy values created during parsing of deftemplates
           are added they do not have a valid 'whichDeftemplate' yet --
           this is not added until the entire deftemplate is
        ...
        */
        if ((fv->whichDeftemplate != NULL) &&
          ...
          {
            /* and array values must be the same */
            for (i=0; i<fv->n; i++)
               {
                 if ((peekfv->x[i] != fv->x[i]) || (peekfv->y[i] != fv->y[i]))
                    break;
               }
            if (i == fv->n) /* found it */
               { return(peek); }
    ...
    /*===================================================*/
    /* Add the fuzzy value at the end of list of entries */
    /* for this hash location.  Return the address of    */
    /* the fuzzy value.                                  */
    /*===================================================*/

    peek = get_struct(theEnv,clipsFuzzyValue);
    ...
    peek->count = 0;
    peek->permanent = false;
    peek->header.type = FUZZY_VALUE_TYPE;

    AddEphemeralHashNode(theEnv,(GENERIC_HN *) peek,&UtilityData(theEnv)->CurrentGarbageFrame->ephemeralFuzzyValueList,
                         sizeof(CLIPSFuzzyValue),AVERAGE_FUZZY_VALUE_SIZE,true);
    UtilityData(theEnv)->CurrentGarbageFrame->dirty = true;
...
size_t HashFuzzyValue(
  ...
  size_t range)
  {
    union
     {
       ...
       struct
        {
         unsigned long int liv1;
         unsigned long int liv2;
         } inner;
     } fis;
   ...
   for (i=0; i<fv->n; i++)
     {
      fis.inner.liv1 = fis.inner.liv2 = 0;
      fis.fv = fv->y[i];
      count += fis.inner.liv1 + fis.inner.liv2;
      fis.inner.liv1 = fis.inner.liv2 = 0;
      fis.fv = fv->x[i];
      count += fis.inner.liv1 + fis.inner.liv2;
     }
...
/*   Adds the fuzzy Value to the EphemeralFuzzyValueList if the count becomes zero.  */
/*                                                                                   */
/*************************************************************************************/
void DecrementFuzzyValueCount( // TBD Rename
  Environment *theEnv,
  CLIPSFuzzyValue *theValue)
  {
   if (theValue->count < 0)
     {
   ...
   if (theValue->markedEphemeral == false)
     {
      AddEphemeralHashNode(theEnv,(GENERIC_HN *) theValue,&UtilityData(theEnv)->CurrentGarbageFrame->ephemeralFuzzyValueList,
                           sizeof(CLIPSFuzzyValue),AVERAGE_FUZZY_VALUE_SIZE,true);
      UtilityData(theEnv)->CurrentGarbageFrame->dirty = true;
     }
...
#if FUZZY_DEFTEMPLATES
   else if (type == FUZZY_VALUE_TYPE)
     {
      rtnFuzzyValue(theEnv,((CLIPSFuzzyValue *) theValue)->contents);
     }
#endif
...
#if FUZZY_DEFTEMPLATES
   RemoveEphemeralHashNodes(theEnv,&theGarbageFrame->ephemeralFuzzyValueList,(GENERIC_HN **) SymbolData(theEnv)->FuzzyValueTable,
                            sizeof(CLIPSFuzzyValue),FUZZY_VALUE_TYPE,AVERAGE_FUZZY_VALUE_SIZE);
#endif
  }
   ...
   void *theValue)
   {
    ...
    switch (((TypeHeader *) theValue)->type)
      ...
      case FUZZY_VALUE_TYPE:
        theFuzzyValue = (CLIPSFuzzyValue *) theValue;
        if (theFuzzyValue->markedEphemeral) return;
        AddEphemeralHashNode(theEnv,(GENERIC_HN *) theValue,
                             &UtilityData(theEnv)->CurrentGarbageFrame->ephemeralFuzzyValueList,
                             sizeof(CLIPSFuzzyValue),AVERAGE_FUZZY_VALUE_SIZE,false);
        UtilityData(theEnv)->CurrentGarbageFrame->dirty = true;
        break;
#endif
...
/*****************************************************************/
CLIPSFuzzyValue **GetFuzzyValueTable(
  Environment *theEnv)
  {
...
/**************************************************************/
void SetFuzzyValueTable(
  Environment *theEnv,
  CLIPSFuzzyValue **value)
  {
