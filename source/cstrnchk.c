...
#if FUZZY_DEFTEMPLATES
   /* fuzzy values can only go where fuzzy values are allowed...
      ie. anyAllowed does not mean a fuzzy value!!
   */
   if ((constraints->anyAllowed == true) &&
       (functionReturnType & (~ FUZZY_BIT)))
     { return true; }
#else
   ...
   if (constraints->fuzzyValuesAllowed)
     { if (functionReturnType & FUZZY_BIT) return true; }
#endif
...
#if FUZZY_DEFTEMPLATES
   if ((type == FUZZY_VALUE_TYPE) && (constraints->fuzzyValuesAllowed != true))
     { return false; }
#endif
   ...
   if (type == FUZZY_VALUE_TYPE)
     {
       ...
       deftPtr = (Deftemplate *) tmpPtr->value;
       fv = ((CLIPSFuzzyValue *) vPtr)->contents;
...
#if FUZZY_DEFTEMPLATES
       (! theConstraint->fuzzyValuesAllowed) &&
#endif
