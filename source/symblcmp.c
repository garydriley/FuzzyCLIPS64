...
#if FUZZY_DEFTEMPLATES
   static unsigned int                FuzzyValueHashNodesToCode(Environment *,const char *,const char *,char *,unsigned int);
   static unsigned int                FuzzyValuesToCode(Environment *,const char *,const char *,char *,unsigned int);
   static unsigned int                FuzzyValueArraysToCode(Environment *,const char *,const char *,char *,unsigned int);
#endif
...
void AtomicValuesToCode(
   ...
   */
   version = SymbolHashNodesToCode(theEnv,fileName,pathName,fileNameBuffer,6);
#else
...
#if FUZZY_DEFTEMPLATES
   version = BitMapValuesToCode(theEnv,fileName,pathName,fileNameBuffer,version);
   version = FuzzyValueHashNodesToCode(theEnv,fileName,pathName,fileNameBuffer,version);
   version = FuzzyValuesToCode(theEnv,fileName,pathName,fileNameBuffer,version);
   version = FuzzyValueArraysToCode(theEnv,fileName,pathName,fileNameBuffer,version);
#else
...
/***********************************************************************/
static unsigned int FuzzyValueHashNodesToCode(
  Environment *theEnv,
  const char *fileName,
  const char *pathName,
  char *fileNameBuffer,
  unsigned int version)
  {
   ...
   for (i = 1; i <= (numberOfEntries / ConstructCompilerData(theEnv)->MaxIndices) + 1 ; i++)
     { fprintf(ConstructCompilerData(theEnv)->HeaderFP,"extern struct clipsFuzzyValue V%d_%d[];\n",ConstructCompilerData(theEnv)->ImageID,i); }
      ...
      while (hashPtr != NULL)
        {
         if (newHeader)
           {
            fprintf(fp,"struct clipsFuzzyValue V%d_%d[] = {\n",ConstructCompilerData(theEnv)->ImageID,arrayVersion);
            newHeader = false;
           }

         fprintf(fp,"{{FUZZY_VALUE_TYPE},");

         /* fill in 'next' slot in clipsFuzzyValue struct */
         ...
         /* fill in other slots in clipsFuzzyValue struct */
         fprintf(fp,"%ld,1,0,0,%d,(struct fuzzy_value *) &W%d_%d[%d]",
                     hashPtr->count + 1,i,
...
/***********************************************************************/
static unsigned int FuzzyValuesToCode(
  Environment *theEnv,
  const char *fileName,
  const char *pathName,
  char *fileNameBuffer,
...
/***********************************************************************/
static unsigned int FuzzyValueArraysToCode(
  Environment *theEnv,
  const char *fileName,
  const char *pathName,
  char *fileNameBuffer,
...
static int HashTablesToCode(
  ...
  char *fileNameBuffer)
  {
   ...
   if ((fp = NewCFile(theEnv,fileName,pathName,fileNameBuffer,1,5,false)) == NULL) return(0);

   fprintf(ConstructCompilerData(theEnv)->HeaderFP,"extern struct clipsFuzzyValue *fvht%d[];\n",ConstructCompilerData(theEnv)->ImageID);
   fprintf(fp,"struct clipsFuzzyValue *fvht%d[%d] = {\n",ConstructCompilerData(theEnv)->ImageID,FUZZY_VALUE_HASH_SIZE);
...
/************************************************************/
void PrintFuzzyValueReference(
  Environment *theEnv,
  FILE *fp,
  CLIPSFuzzyValue *fvPtr)
  {
   if (fvPtr == NULL) fprintf(fp,"NULL");
   else fprintf(fp,"&V%u_%u[%u]",ConstructCompilerData(theEnv)->ImageID,
                                 (fvPtr->bucket / ConstructCompilerData(theEnv)->MaxIndices) + 1,
                                  fvPtr->bucket % ConstructCompilerData(theEnv)->MaxIndices);
  }
