   ...
   switch(bexp->type)
     {
      ...
      case FUZZY_VALUE_TYPE:
        ...
        IncrementFuzzyValueCount(ExpressionData(theEnv)->ExpressionArray[obji].fuzzyValue);
        break;
#endif
   ...
   for (i = 0; i < ExpressionData(theEnv)->NumberOfExpressions; i++)
     {
      ...
      case FUZZY_VALUE_TYPE:
               ...
               /* NOTE: this stuff is being done to clear an already bloaded system
                           so another bloaded system or a standard system can then
                                   be loaded
               */

          DecrementFuzzyValueCount(theEnv,ExpressionData(theEnv)->ExpressionArray[i].fuzzyValue);
...
#if FUZZY_DEFTEMPLATES
      case FUZZY_VALUE_TYPE:
           newTest.value = testPtr->fuzzyValue->bucket;
           break;
#endif
