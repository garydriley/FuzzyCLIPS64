...
void PrimitiveTablesInfoCommand(
  ...
  UDFValue *returnValue)
  {
   ...
   CLIPSFuzzyValue **fuzzyValueArray, *fuzzyValuePtr;
   unsigned long int fuzzyValueCount = 0;
#endif
   ...
   for (i = 0; i < FUZZY_VALUE_HASH_SIZE; i++)
     {
      for (fuzzyValuePtr = fuzzyValueArray[i]; fuzzyValuePtr != NULL; fuzzyValuePtr = fuzzyValuePtr->next)
        { fuzzyValueCount++; }
     }
#endif
   ...
   WriteString(theEnv,STDOUT,"\n");
#if FUZZY_DEFTEMPLATES
   WriteString(theEnv,STDOUT,"FuzzyValues: ");
   PrintUnsignedInteger(theEnv,STDOUT,fuzzyValueCount);
   WriteString(theEnv,STDOUT,"\n");
#endif
  }
