...
/* FuzzySlotAnalysis:                                             */
/*                                                                */
/* Number of FuzzySlots in the patterns of the rule is returned.  */
/* Number of FuzzySlots in patterns that are non Not patterns also*/
/* returned via the 2nd parameter -- numFuzzySlotsInNonNotPatterns*/
/*                                                                */
/* If Fuzzy Pattern (pattern->bottom->type is FUZZY_VALUE)        */
/* then create an 'expr' of type SCALL_PN_FUZZY_VALUE with argList*/
/* slot pointing to a FUZZY_VALUE expression slot                 */
/* (CLIPSFuzzyValue) and 2nd arg is the slot number               */
/*                                                                */
...
/******************************************************************/
unsigned int FuzzySlotAnalysis(
  Environment *theEnv,
       ...
       while (lhsSlotPtr != NULL)
         {
          if (lhsSlotPtr->bottom != NULL &&
              lhsSlotPtr->bottom->pnType == FUZZY_VALUE_NODE)
            {
             /* if not a negated pattern count fuzzy slots
                eg. not (Temperature cold)  is not a fuzzy pattern
                    since the fuzzy fact must not exist for the
                    pattern to match
             */
             if (!lhsPNPtr->negated)
                *numFuzzySlotsInNonNotPatterns = *numFuzzySlotsInNonNotPatterns + 1;

             numFuzzySlots++;

             fz_expr = get_struct(theEnv,expr);
             fz_expr->type = SCALL_PN_FUZZY_VALUE;
             fz_expr->value = NULL;
             fz_expr->argList = get_struct(theEnv,expr);
             fz_expr->argList->type = FUZZY_VALUE_TYPE;
             fz_expr->argList->argList = NULL;
             fz_expr->argList->value = (void *)lhsSlotPtr->bottom->value;
             fz_expr->argList->nextArg =
                   GenConstant(theEnv,INTEGER_TYPE,CreateInteger(theEnv,lhsSlotPtr->slotNumber-1));
             fz_expr->nextArg = NULL;
             lhsSlotPtr->bottom->value = NULL;

             lhsSlotPtr->networkTest =
                   CombineExpressions(theEnv,fz_expr,lhsSlotPtr->networkTest);
             ReturnLHSParseNodes(theEnv,lhsSlotPtr->bottom);
